...
- **上下文窗口使用显示**：输入区底部实时显示最后一轮对话（请求输入 + 回复输出）占模型上下文窗口的百分比与窗口大小（K 记法）；API 未返回 token 用量时按消息文本自动估算，并在数值后标注「（估算）」
- **自动上下文压缩**：发送新消息前预估请求输入超过上下文窗口 80% 时，先调用 LLM 把之前的对话压缩成摘要，之后基于「压缩摘要 + 后续消息」继续对话；压缩记录显示在对话中（默认仅一行，点击可展开查看压缩全文，并显示该轮压缩请求的输入/输出 token）；记录持久化在消息树中，与分叉/分支切换兼容
- **多模型管理**：可添加/编辑/删除多个模型配置（endpoint + 模型名 + key + 上下文窗口大小，缺省 128K），聊天中随时切换
